/**********************************************************************************************
* This program clusters the input data in arff by using Bisecting k-means algorithm. It can
* cluster original data or normalized data. If given, intended class attribute can be
* avoided while clustering. Two output files are created while executing. One output file holds
* the cluster centeres along with the cluster ids. Another output file holds the input data
* along with cluster ID where it belongs to. If the clustering is performed on normalized data
* then 4 output files are created. Two additional files remaps the cluster centers and input
* data back.
*************************************************************************************************/
#include <iostream>
#include <iomanip>
...
void assignClusterID(int numOfAttributes, int eachAttrLength, int clustNumToBisect) {
    ...
    for (int pos = 0; pos < eachAttrLength; pos++){
        if (((!Normalize) && ((inputAttributes[numOfAttributes - 1].attrValues[pos] == clustNumToBisect)
            || (inputAttributes[numOfAttributes - 1].attrValues[pos] == currentClusters - 1)))
            || ((Normalize) && ((inputAttributes[numOfAttributes - 1].NormValues[pos] == clustNumToBisect)
            || (inputAttributes[numOfAttributes - 1].NormValues[pos] == currentClusters - 1)))) {
...
void doKmeansClustering(int numOfAttributes, int eachAttrLength, int clustNumToBisect) {
    ...
    while (goOn){
        ...
        // are the centroids of last calculation and this matches? If yes, terminate
...
void openFile()  {
    ...
    if (!Normalize){
        if (!userOutputFileName){
            Cluster_Center_Bisect_OutFileName = "Bisecting_Kmeans_ClusterCenters_" + std::to_string(numOfClusters) + "_" + inputFileName;
            Clustering_Bisect_OutFileName = "Bisecting_Kmeans_Clustering_" + std::to_string(numOfClusters) + "_" + inputFileName;
        }
        else{
            Cluster_Center_Bisect_OutFileName = "ClusterCenters_" + outputFileName;
            Clustering_Bisect_OutFileName = outputFileName;
        }
        ...
        if (!userOutputFileName){
            Cluster_Center_Bisect_OutFileName = "Bisecting_Kmeans_ClusterCenters_Normalized_" + std::to_string(numOfClusters) + "_" + inputFileName;
            Clustering_Bisect_OutFileName = "Bisecting_Kmeans_Clustering_Normalized_" + std::to_string(numOfClusters) + "_" + inputFileName;
            Cluster_Center_UnnormBisect_OutFileName = "Bisecting_Kmeans_ClusterCenters_Unnormalized_" + std::to_string(numOfClusters) + "_" + inputFileName;
            Clustering_UnnormBisect_OutFileName = "Bisecting_Kmeans_Clustering_Unnormalized_" + std::to_string(numOfClusters) + "_" + inputFileName;
        }
        else{
            Cluster_Center_Bisect_OutFileName = "ClusterCenters_Normalized_" + outputFileName;
            Clustering_Bisect_OutFileName = "Normalized_" + outputFileName;
            Cluster_Center_UnnormBisect_OutFileName = "ClusterCenters_Unnormalized_" + outputFileName;
            Clustering_UnnormBisect_OutFileName = "Unnormalized_" + outputFileName;
        }
